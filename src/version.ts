...
function readPackageMetadata(): PackageMetadata {
  ...
  if (!raw || typeof raw !== "object") throw new Error("Agent Lounge package metadata is invalid.");
  const candidate = raw as Record<string, unknown>;
  if (typeof candidate.name !== "string" || typeof candidate.version !== "string") {
    throw new Error("Agent Lounge package metadata is missing its name or version.");
  }
